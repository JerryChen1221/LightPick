...
# Cloudflare D1 Setup Script for LightPick
# This script automates the creation and setup of D1 databases

set -e  # Exit on error

echo "🚀 LightPick - Cloudflare D1 Setup"
echo "======================================"
echo ""
...
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "Creating database: lightpick-frontend..."
    wrangler d1 create lightpick-frontend
    ...
    wrangler d1 execute lightpick-frontend --remote --file=frontend/drizzle/d1-schema.sql
...
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "Creating database: lightpick-backend..."
    wrangler d1 create lightpick-backend
    ...
    wrangler d1 execute lightpick-backend --remote --file=backend/migrations/d1/0001_checkpoints.sql
...
echo "   wrangler d1 execute lightpick-frontend --remote --command \"SELECT name FROM sqlite_master\""
echo "   wrangler d1 execute lightpick-backend --remote --command \"SELECT name FROM sqlite_master\""
echo ""
