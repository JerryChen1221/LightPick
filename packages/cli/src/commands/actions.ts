...
const REGISTRY_URL = "https://raw.githubusercontent.com/lightpick-community/awesome-actions/main/registry.json";
    ...
    try {
      ...
      if (isJsonMode(options)) {
        ...
        if (manifest.secrets?.length) {
          ...
          console.log(`  → Set variables with: lightpick vars set <KEY>`);
        }
      }
    } finally {
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (isJsonMode(options)) {
        ...
        console.log("No actions installed. Use `lightpick action install` to add one.");
      } else {
