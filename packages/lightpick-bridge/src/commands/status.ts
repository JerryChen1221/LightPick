/**
 * `lightpick-bridge status` — print local creds + (best-effort) ping the server
 ...
 * actually running" the user can check `launchctl list | grep lightpick`
...
export async function runStatus(): Promise<void> {
  ...
  if (!creds) {
    log.warn("not set up — run `lightpick-bridge setup` to register this machine");
