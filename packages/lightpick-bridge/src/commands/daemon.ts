/**
 * `lightpick-bridge daemon` — long-running reverse-WS to the control plane.
 *
...
export async function runDaemon(): Promise<void> {
  ...
  if (!creds) {
    process.stderr.write(
      "✗ no credentials. Run `lightpick-bridge setup` first.\n",
    );
    process.exit(2);
  }
  ...
  // it fails the daemon still starts; user can clean ~/.lightpick/sessions/
  ...
  // spawned ACP child as LIGHTPICK_API_KEY so its bundled `lightpick` CLI /
  // plugin hooks can authenticate without prompting the user.
  sessions.setSpawnEnv({
    LIGHTPICK_API_KEY: creds.agentApiKey,
    LIGHTPICK_API_URL: creds.serverUrl,
  });
