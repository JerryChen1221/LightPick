/**
 * `lightpick-bridge setup` — one-time onboarding.
 *
 *   1. Bind 127.0.0.1:<rand-port> as a single-shot HTTP server.
 *   2. Open the user's browser to https://lightpick.video/connect-daemon?cb=…&state=…
...
interface SetupOpts {
   ...
   *  zone (lightpick.video) rather than the API zone (api.lightpick.video). */
...
export async function runSetup(opts: SetupOpts): Promise<void> {
  ...
  // `npx @lightpick/bridge@beta setup` a clean upgrade flow: same one
  ...
  if (!opts.force) {
    ...
    if (existing) {
      ...
      if (!opts.noService && currentPlatform() === "darwin") {
        ...
        log.hint("run `lightpick-bridge daemon` to start the bridge");
      }
  ...
  if (opts.noService || currentPlatform() !== "darwin") {
    ...
    log.hint("run `lightpick-bridge daemon` to start the bridge in the foreground");
    return;
  }
