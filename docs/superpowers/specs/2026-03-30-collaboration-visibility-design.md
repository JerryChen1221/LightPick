...
When an external agent (via CLI) edits a LightPick project canvas, the browser user has no awareness of what's happening. Loro CRDT syncs the data, but there's no indication of *who* made changes or *what* is being done. This makes multi-client collaboration feel invisible.
...
1. **Presence**: Open project in browser → run `lightpick canvas list --project <id>` → see "CLI Agent" avatar appear in toolbar → CLI disconnects → avatar disappears
2. **Activity highlight**: Open project in browser → run `lightpick canvas add --project <id> --type text --label "Test"` → see node appear with blue ring + "CLI Agent added" label → fades after 3s
